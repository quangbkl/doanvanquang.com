...
file_put_contents("message.txt", json_encode($message));
file_put_contents("type.txt", json_encode($msg_typemsg_type));


$log_sender = file_get_contents("log_sender.txt");
if (strpos($log_sender, $sender) === FALSE) {
	$save_log_sender = fopen("log_sender.txt", "a");
	fwrite($save_log_sender, $sender." ");
	fclose($save_log_sender);
}
